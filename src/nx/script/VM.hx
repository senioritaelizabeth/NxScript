...
// usingExtensions init removed
	...
	public function callFunction(func:FunctionChunk, closure:Map<String, Value>, args:Array<Value>):Value {
		...
		// Save VM instance state — callFunction may be invoked re-entrantly from inside
		// a native callback (e.g. map/filter/forEach/sort script lambdas).
		// Without save/restore, frames = [funcFrame] destroys the outer call stack and
		// run() returns leaving the outer run() loop with a dead frames array.
		var savedFrames       = this.frames;
		var savedCurrentFrame = this.currentFrame;
		var savedSp           = this.sp;
		...
		// run() caches chunk/code/constants/strings/frameBase from currentFrame at its
		// own start — the outer run() loop re-syncs those from its own local variables
		// which remain untouched, so we only need to restore the instance fields here.
		var result = run();
	...
	function getNumberMethod(n:Float, method:String):Value {
		return switch (method) {
			...
			// Aliases (JS/NxScript-friendly names)
			case "toInt":    VNativeFunction("toInt",    0, (_) -> VNumber(Math.ffloor(n)));
			case "toFloat":  VNativeFunction("toFloat",  0, (_) -> VNumber(n));
			case "toString": VNativeFunction("toString", 0, (_) -> VString(Std.string(n)));
			case "toBool":   VNativeFunction("toBool",   0, (_) -> VBool(n != 0));

			// Utilities
			case "clamp": VNativeFunction("clamp", 2, (args) -> {
					var lo = switch (args[0]) { case VNumber(x): x; default: throw 'clamp: expected number for lo'; };
					var hi = switch (args[1]) { case VNumber(x): x; default: throw 'clamp: expected number for hi'; };
					VNumber(Math.max(lo, Math.min(hi, n)));
				});
			case "sign":     VNativeFunction("sign",     0, (_) -> VNumber(n > 0 ? 1.0 : n < 0 ? -1.0 : 0.0));
			case "lerp": VNativeFunction("lerp", 2, (args) -> {
					var b = switch (args[0]) { case VNumber(x): x; default: throw 'lerp: expected number for b'; };
					var t = switch (args[1]) { case VNumber(x): x; default: throw 'lerp: expected number for t'; };
					VNumber(n + (b - n) * t);
				});
			case "isNaN":    VNativeFunction("isNaN",    0, (_) -> VBool(Math.isNaN(n)));
			case "isFinite": VNativeFunction("isFinite", 0, (_) -> VBool(Math.isFinite(n)));
			case "log":      VNativeFunction("log",      0, (_) -> VNumber(Math.log(n)));
			case "log2":     VNativeFunction("log2",     0, (_) -> VNumber(Math.log(n) / Math.log(2)));
			case "log10":    VNativeFunction("log10",    0, (_) -> VNumber(Math.log(n) / Math.log(10)));
			case "exp":      VNativeFunction("exp",      0, (_) -> VNumber(Math.exp(n)));
			case "atan2": VNativeFunction("atan2", 1, (args) -> switch (args[0]) {
					case VNumber(x): VNumber(Math.atan2(n, x));
					default: throw 'atan2: expected number';
				});
